...
class Bottleneck3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None, kernel_size=(2, 3, 3), dilation=(1, 1, 1),
        ...
        self.layers = nn.Sequential(OrderedDict([
            ...
            # Second conv block
            ('conv', CausalConv3d(bottleneck_channels, bottleneck_channels, kernel_size=kernel_size,
...
class PyramidSpatioTemporalPooling(nn.Module):
    ...
    def __init__(self, in_channels, reduction_channels, pool_sizes):
        ...
        for pool_size in pool_sizes:
            assert pool_size[0] == 2, "Time kernel should be 2 as PyTorch raises an error when" \
                                        "padding with more than half the kernel size"
            stride = (1, *pool_size[1:])
            padding = (pool_size[0] - 1, 0, 0)
            self.features.append(nn.Sequential(OrderedDict([
                # Pad the input tensor but do not take into account zero padding into the average.
                ('avgpool', torch.nn.AvgPool3d(kernel_size=pool_size, stride=stride, padding=padding,
    ...
    def forward(self, *inputs):
        ...
        for f in self.features:
            ...
            c = x_pool.shape[1]
            x_pool = nn.functional.interpolate(x_pool.view(b * t, c, *x_pool.shape[-2:]), (h, w), mode='bilinear',
                                               align_corners=False)
...
class TemporalBlock(nn.Module):
    """ Temporal block with the following layers:
        - 2x3x3, 2x1x3, 2x3x1, 1x3x3, 1x1x1, spatio-temporal pyramid pooling
        - dropout
        - skip connection.
    """
    def __init__(self, in_channels, out_channels=None, use_pyramid_pooling=False,
                 pool_sizes=None):
        super().__init__()
        self.in_channels = in_channels
        self.half_channels = in_channels // 2
        self.out_channels = out_channels or self.in_channels
        self.kernels = [(2, 3, 3), (2, 1, 3), (2, 3, 1), (1, 3, 3)]

        # Flag for spatio-temporal pyramid pooling
        self.use_pyramid_pooling = use_pyramid_pooling

        # 4 convolution paths: 2x3x3, 2x1x3, 2x3x1, 1x3x3, 1x1x1
        self.convolution_paths = []
        for kernel_size in self.kernels:
            self.convolution_paths.append(nn.Sequential(
        ...
        if self.use_pyramid_pooling:
            ...
            reduction_channels = self.in_channels // len(pool_sizes)
